...
import { validateEmail, validateUsername, validatePassword } from "./schema";
...
export const action: ActionFunction = async ({ request }) => {
  ...
  const errors = {
    username: validateUsername.safeParse(username),
  ...
  if (Object.values(errors).some((error) => error.success === false)) {
    return json(
      {
        errors: {
          username: errors.username.error?.issues[0]?.message,
